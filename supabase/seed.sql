...
-- API Keysテーブルにデータを挿入
INSERT INTO public.api_keys (user_id, name, description, encrypted_key, is_active, created_at, last_used, expires_at) VALUES
...
((SELECT id FROM public.api_keys WHERE encrypted_key = 'prod_key_123456'), '/api/v1/users', 50, CURRENT_DATE),
((SELECT id FROM public.api_keys WHERE encrypted_key = 'prod_key_123456'), '/api/v1/products', 30, CURRENT_DATE),
((SELECT id FROM public.api_keys WHERE encrypted_key = 'dev_key_789012'), '/api/v1/orders', 20, CURRENT_DATE);